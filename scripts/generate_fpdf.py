...
def clean_comp_label(label):
    s = str(label)
    s = s.replace("Oracle Corporation ", "")
...
def generate_charts(df):
    ...
    ax.set_title('Vulnerability Distribution by CVSS Severity Rating', fontsize=10.5, fontweight='bold', pad=10, color='#0f172a')
    ax.set_xlabel('Number of Vulnerabilities', fontsize=8.5, color='#334155')
    for bar in bars:
        w = bar.get_width()
    ...
    ax.set_title('Vulnerability Exploitation Status in the Wild', fontsize=10.5, fontweight='bold', pad=10, color='#0f172a')
    ax.set_xlabel('Count', fontsize=8.5, color='#334155')
    for bar in bars:
        w = bar.get_width()
        ax.text(w + 12, bar.get_y() + bar.get_height()/2, f'{int(w)} ({w/len(df)*100:.1f}%)', 
                va='center', ha='left', fontsize=8, fontweight='bold', color='#1e293b')
    ax.set_xlim(0, max(exp_counts.values) * 1.18)
    plt.tight_layout()
    ...
    bars = ax.barh(sector_counts.index[::-1], sector_counts.values[::-1], color=colors[:len(sector_counts)], height=0.56)
    ax.set_title('Vulnerability Exposure by Target Sector in India', fontsize=10.5, fontweight='bold', pad=10, color='#0f172a')
    ax.set_xlabel('Number of Impacted Vulnerabilities', fontsize=8.5, color='#334155')
    for bar in bars:
        w = bar.get_width()
    ...
    cleaned_index = [clean_comp_label(k) for k in comp_counts.index]
    bars = ax.barh(cleaned_index[::-1], comp_counts.values[::-1], color='#4f46e5', height=0.56)
    ax.set_title('Top Affected Software & Technology Stacks', fontsize=10.5, fontweight='bold', pad=10, color='#0f172a')
    ax.set_xlabel('Vulnerability Count', fontsize=8.5, color='#334155')
    for bar in bars:
        w = bar.get_width()
...
def build_pdf():
    ...
    pdf.set_font("ArialCustom", "B", 16)
    pdf.cell(0, 9, "Table of Contents & Executive Roadmap", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    toc_items = [
        ...
        ("3. Section I: Active Attack Forensic Profiles (37 Records)", "5", "Exhaustive forensic attack profiles of all 37 vulnerabilities confirmed exploited in the wild."),
        ("4. Section II: Comprehensive Vulnerability Audit Registry (All 1,273 Records)", "14", "Systematic technical record catalog of each and every audited vulnerability with full details."),
    ...
    for title, pg, desc in toc_items:
        ...
        pdf.cell(160, 6.5, title, fill=True, new_x=XPos.RIGHT, new_y=YPos.TOP)
        pdf.set_font("ArialCustom", "B", 9.5)
        pdf.set_text_color(71, 85, 105)
        pdf.cell(0, 6.5, f"Page {pg}", fill=True, align="R", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_x(14)
    ...
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 7, "Authoritative Verification Standards & Compliance", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.multi_cell(w=pdf.epw, h=4.3, text=compliance_text, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.set_font("ArialCustom", "B", 16)
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 9, "1. Executive Summary & National Threat Landscape", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_draw_color(37, 99, 235)
    pdf.line(12, pdf.get_y(), 198, pdf.get_y())
    pdf.ln(4)
    ...
    pdf.multi_cell(w=pdf.epw, h=4.4, text=exec_summary_1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(3)

    # KPI Metric Boxes (FIXED HORIZONTAL ALIGNMENT)
    box_w = 44
    box_h = 18
    base_kpi_y = pdf.get_y()
    ...
    for i, (label, val, color) in enumerate(kpi_data):
        x_pos = 12 + i * 46.5
        pdf.rect(x_pos, base_kpi_y, box_w, box_h, "DF")
        
        pdf.set_xy(x_pos, base_kpi_y + 2)
        pdf.set_font("ArialCustom", "B", 13)
        pdf.set_text_color(*color)
        pdf.cell(box_w, 6, val, align="C", new_x=XPos.RIGHT, new_y=YPos.TOP)
        
        pdf.set_xy(x_pos, base_kpi_y + 9)
        pdf.set_font("ArialCustom", "B", 7)
        pdf.set_text_color(100, 116, 139)
        pdf.cell(box_w, 5, label, align="C", new_x=XPos.RIGHT, new_y=YPos.TOP)

    pdf.set_y(base_kpi_y + box_h + 4)

    pdf.set_font("ArialCustom", "B", 11)
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 7, "Primary Attack Vectors Observed Across Indian Infrastructure", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.set_font("ArialCustom", "", 8.5)
    pdf.set_text_color(51, 65, 85)
    pdf.multi_cell(w=pdf.epw, h=4.3, text=vectors_text, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.set_font("ArialCustom", "B", 16)
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 9, "2. Comprehensive Statistical & Sector Dashboards", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_draw_color(37, 99, 235)
    pdf.line(12, pdf.get_y(), 198, pdf.get_y())
    pdf.ln(4)

    chart_y = pdf.get_y()
    pdf.image("images/severity_distribution.png", x=12, y=chart_y, w=89)
    pdf.image("images/exploitation_status.png", x=105, y=chart_y, w=89)
    
    chart_y2 = chart_y + 60
    pdf.image("images/sector_distribution.png", x=12, y=chart_y2, w=89)
    pdf.image("images/top_components.png", x=105, y=chart_y2, w=89)
    
    pdf.set_y(chart_y2 + 65)

    pdf.set_font("ArialCustom", "B", 10.5)
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 6, "Sectoral Vulnerability Distribution Analysis", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.set_font("ArialCustom", "", 8)
    pdf.set_text_color(51, 65, 85)
    pdf.multi_cell(w=pdf.epw, h=4.1, text=sector_analysis, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.cell(0, 9, "3. Section I: Active Attack Forensic Profiles (37 Records)", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.multi_cell(w=pdf.epw, h=4.0, text=sec1_intro, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(3)
    ...
    for idx, row in active_df.iterrows():
        ...
        pdf.rect(12, card_start_y, 186, 6.5, "DF")
        
        pdf.set_xy(14, card_start_y + 0.8)
        ...
        pdf.set_font("ArialCustom", "B", 7.5)
        pdf.set_text_color(71, 85, 105)
        pdf.cell(0, 4.5, f"Component: {row['Affected_Component']}  |  Sector: {row['Target_Sector_India']}  |  Classification: {row['Vulnerability_Classification']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)

        # Attack summary
        pdf.set_x(14)
        pdf.set_font("ArialCustom", "", 7.5)
        pdf.set_text_color(30, 41, 59)
        pdf.multi_cell(w=182, h=3.8, text=f"Attack Mechanics & Technical Vector: {row['Technical_Summary']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)

        # Remediation
        pdf.set_x(14)
        pdf.set_font("ArialCustom", "B", 7)
        pdf.set_text_color(185, 28, 28)
        pdf.multi_cell(w=182, h=3.6, text=f"Action Required: {row['Remediation_Vector']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)

        # Reference
        pdf.set_x(14)
        pdf.set_font("ArialCustom", "I", 7)
        pdf.set_text_color(100, 116, 139)
        pdf.multi_cell(w=182, h=3.4, text=f"Advisory Link: {row['Verified_Source_Reference']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.set_font("ArialCustom", "B", 15)
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 9, "4. Section II: Comprehensive Vulnerability Audit Registry", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_draw_color(37, 99, 235)
    pdf.line(12, pdf.get_y(), 198, pdf.get_y())
    pdf.ln(3)
    ...
    pdf.set_font("ArialCustom", "", 8)
    pdf.set_text_color(51, 65, 85)
    pdf.multi_cell(w=pdf.epw, h=4.0, text=sec2_intro, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(3)

    # Stream ALL 1,273 records with non-clipped multi_cells
    for i, row in df.iterrows():
        # Estimate height: Header (5) + Meta (4) + Desc (~8) + Remediation (~6) + spacing ~ 25-30 mm
        if pdf.get_y() > 248:
            pdf.add_page()
        ...
        if is_active:
            ...
            pdf.set_text_color(30, 58, 138)
        pdf.cell(28, 5.2, f"{row['Record_ID']}", fill=True, new_x=XPos.RIGHT, new_y=YPos.TOP)
        
        pdf.set_text_color(15, 23, 42)
        pdf.cell(38, 5.2, f"{row['Vulnerability_Identifier']}", fill=True, new_x=XPos.RIGHT, new_y=YPos.TOP)
        
        if "Critical" in str(row['Severity_CVSS']):
            pdf.set_text_color(220, 38, 38)
        else:
            pdf.set_text_color(217, 119, 6)
        pdf.cell(45, 5.2, f"{row['Severity_CVSS']}", fill=True, new_x=XPos.RIGHT, new_y=YPos.TOP)
        
        pdf.set_font("ArialCustom", "B", 7)
        pdf.set_text_color(*badge_color)
        pdf.cell(0, 5.2, badge_text, fill=True, align="R", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        pdf.cell(w=pdf.epw, h=4.2, text=comp_str, new_x=XPos.LMARGIN, new_y=YPos.NEXT)

        # Technical Summary
        pdf.set_x(12)
        pdf.set_font("ArialCustom", "", 7.2)
        pdf.set_text_color(30, 41, 59)
        pdf.multi_cell(w=pdf.epw, h=3.6, text=f"Description: {row['Technical_Summary']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)

        # Remediation & Reference (WRAPPED PROPERLY)
        pdf.set_x(12)
        pdf.set_font("ArialCustom", "I", 6.8)
        pdf.set_text_color(100, 116, 139)
        pdf.multi_cell(w=pdf.epw, h=3.3, text=f"Remediation: {row['Remediation_Vector']}  |  Ref: {row['Verified_Source_Reference']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)

        # Divider line
        pdf.set_draw_color(226, 232, 240)
        pdf.line(12, pdf.get_y() + 1.2, 198, pdf.get_y() + 1.2)
    ...
    pdf.set_font("ArialCustom", "B", 15)
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 9, "5. Section III: Strategic Defense & Incident Response Directives", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_draw_color(37, 99, 235)
    pdf.line(12, pdf.get_y(), 198, pdf.get_y())
    pdf.ln(4)
    ...
    pdf.set_font("ArialCustom", "B", 10.5)
    pdf.set_text_color(30, 58, 138)
    pdf.cell(0, 6.5, "A. Mandatory Remediation SLAs for Indian Cyberspace Entities", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.set_font("ArialCustom", "", 8)
    pdf.set_text_color(51, 65, 85)
    pdf.multi_cell(w=pdf.epw, h=4.2, text=sla_text, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(3.5)

    pdf.set_font("ArialCustom", "B", 10.5)
    pdf.set_text_color(30, 58, 138)
    pdf.cell(0, 6.5, "B. Threat Hunting Signatures & Log Auditing Vectors", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.set_font("ArialCustom", "", 8)
    pdf.set_text_color(51, 65, 85)
    pdf.multi_cell(w=pdf.epw, h=4.2, text=hunting_text, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(3.5)

    pdf.set_font("ArialCustom", "B", 10.5)
    pdf.set_text_color(30, 58, 138)
    pdf.cell(0, 6.5, "C. Concluding Attestation & Governance", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.set_font("ArialCustom", "I", 8)
    pdf.set_text_color(71, 85, 105)
    pdf.multi_cell(w=pdf.epw, h=4.2, text=attest_text, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
